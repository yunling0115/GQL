...
public class ShowButton extends Applet implements ActionListener, MouseMotionListener, MouseListener {
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (query_type == "Emergency Query") {
			...
			// show submitted query
			if (c1==true) {String s1=""; queries.add(s1);}
			if (c2==true) {String s2=""; queries.add(s2);}
			if (c3==true) {String s3=""; queries.add(s3);}
			String q = "";
			for (int i=0;i<queries.size();i++) {
				if (i==0) {q = "Query"+(int)(i+1)+": "+queries.get(i); }
				else {q = q+"\n"+"Query"+(int)(i+1)+": "+queries.get(i);}
